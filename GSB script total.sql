...
create table stocker(ID_MAT char(4),
ID_MEDIC varchar(50),
QTESTOCK int,
primary key(ID_MAT, ID_MEDIC),
foreign key(ID_MAT) REFERENCES visiteur(MATRICULE),
foreign key(ID_MEDIC) REFERENCES medicament(MED_DEPOTLEGAL));

create table offrir(REF char(5),
ID_MEDIC varchar(50),
QTEOFFERTE int,
primary key(ID_MEDIC, REF),
foreign key (REF) REFERENCES visite(REFERENCE),
foreign key (ID_MEDIC) REFERENCES medicament(MED_DEPOTLEGAL));